...
class AuthBootstrap:
    ...
    async def _bootstrap_roles(self) -> None:
        ...
        for role_name, scopes in settings.auth_bootstrap.bootstrap_roles.items():
            ...
            description = f"Default {role_name} role" if is_default else f"{role_name} role"
            ...
            logger.info(f"Created/verified role: {role_name} with {len(scopes)} permissions.")
    ...
    async def _bootstrap_admin_user(self) -> None:
        ...
        session = self._role_service._repository._Repository__session

        user_service = UserService(session=session)
        existing_admin = await user_service.get_by_email(settings.auth_bootstrap.admin_email)
        ...
        if not existing_admin:
            ...
            if admin_role:
                await self._role_service.assign_role_to_user(admin_user.id, admin_role.id)
                logger.info(f"Created admin user: {settings.auth_bootstrap.admin_email}")
        else:
            logger.info(f"Admin user already exists: {settings.auth_bootstrap.admin_email}")